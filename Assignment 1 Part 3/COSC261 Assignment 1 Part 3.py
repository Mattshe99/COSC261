...
class If_Else_AST:
    def __init__(self, condition, then, elsee):
        self.condition = condition
        self.then = then
        self.elsee = elsee
    ...
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                       repr(self.then) + ' else ' + repr(self.elsee) + \
                       ' end'
    def indented(self, level):
        return indent('If-Else', level) + \
               self.condition.indented(level+1) + \
               self.then.indented(level+1)+ \
               self.elsee.indented(level+1)
    
class Bltm_AST:
    def __init__(self, hlw):
        self.hlw = hlw
   
    def true_code(self, label):
        l1 = label_generator.next()
        true = ""
        f_list = [i.false_code(l1) for i in self.hlw]
        true = "".join(f_list)
        ret = true + 'goto ' + label + '\n' + l1 + ':\n'
        return ret
    
    def false_code(self,label):
        f_list = [i.false_code(label) for i in self.hlw]
        false = "".join(f_list)
        return false
...
class Blex_AST:
    ...
    def true_code(self, label):
        true = ""
        t_list = [i.true_code(label) for i in self.into]
        true = "".join(t_list)
        return true
    ...
    def false_code(self, label):
        l1 = label_generator.next()
        true = ""
        t_list = [i.true_code(l1) for i in self.into]
        true = "".join(t_list)
        ret = true + 'goto '  + label + '\n' + l1 + ':\n'
        return ret
    
class Blfc_AST:
    def __init__(self, amns):
        self.amns = amns
        
    def true_code(self, label):
        return self.amns.false_code(label)
    
    def false_code(self, label):
        return self.amns.true_code(label)
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        return If_Else_AST(condition, then, elsee)
...
def write():
    ...
    rit = expression()
    return Write_AST(rit)

def bltm():
    l_blfc = []
    l_blfc.append(blfc())
    
    scn_tok = scanner.lookahead()
    while Token.AND == scn_tok:
        scanner.consume(Token.AND)
        l_blfc.append(blfc())
        scn_tok = scanner.lookahead()
    return Bltm_AST(l_blfc)
...
def blex():
    ...
    scn_tok = scanner.lookahead()
    while Token.OR == scn_tok:
        scanner.consume(Token.OR)
        l_bltm.append(bltm())
        scn_tok = scanner.lookahead()
    return Blex_AST(l_bltm)

def blfc():
    if Token.NOT != scanner.lookahead():
        return comparison()
    else:
        scanner.consume(Token.NOT)
        return Blfc_AST(blfc())
